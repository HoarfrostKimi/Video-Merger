...
## 更新摘要
**变更内容**
- 新增排除过滤系统，实现桌面端与移动端界面的实时同步
- 添加 applyExcludeFilter() 函数，应用隐藏文件夹排除到上次扫描结果
- 增强配置保存流程，确保手机端即时反映排除状态变化
- 优化分组管理 API，支持排除/恢复分组的实时同步
...
**更新** 新增排除过滤系统，实现了桌面端与移动端界面的实时同步，用户无需手动刷新即可看到最新的分组状态。
...
- 控制服务器（controlServer.ts）
  ...
  - **新增** applyExcludeFilter() 函数，应用隐藏文件夹排除到 lastGroups
  - **新增** getFilteredGroups() 函数，动态获取过滤后的分组列表
- 主进程（index.ts）
  - 启动控制服务器，注入回调
  - 实现 performScan 扫描逻辑，按标题+时间间隔分组
  - 管理本地文件服务器（为插件提供临时 URL）
  - 暴露 IPC 接口给渲染进程
  - **新增** 在配置保存后调用 applyExcludeFilter() 实现实时同步
- FFmpeg 封装（ffmpeg.ts）
...
**更新** 新增排除过滤系统的实时同步机制，确保桌面端和移动端的分组列表始终保持一致。
...
Note over CS : 新增排除过滤同步
Main->>CS : applyExcludeFilter()
CS->>CS : 应用 hiddenFolderKeys 过滤 lastGroups
CS-->>Phone : 实时更新的分组列表
```
...
- 关键设计点
  ...
  - **新增** 排除过滤系统：applyExcludeFilter() 函数应用隐藏文件夹排除，getFilteredGroups() 动态过滤分组

**更新** 新增排除过滤功能，实现了桌面端与移动端的实时同步：

```mermaid
flowchart TD
Start(["配置保存"]) --> LoadConfig["加载最新配置"]
LoadConfig --> GetHiddenKeys["获取 hiddenFolderKeys"]
GetHiddenKeys --> FilterGroups["过滤 lastGroups"]
FilterGroups --> UpdateState["更新应用状态"]
UpdateState --> NotifyMobile["通知移动端刷新"]
NotifyMobile --> End(["完成"])
subgraph "新增排除过滤流程"
ApplyFilter["applyExcludeFilter()"] --> ReadConfig["读取配置"]
ReadConfig --> CreateSet["创建隐藏键集合"]
CreateSet --> FilterLastGroups["过滤 lastGroups"]
FilterLastGroups --> Return["返回过滤结果"]
end
```

**图表来源**
- [src/main/controlServer.ts:163-167](file://src/main/controlServer.ts#L163-L167)
- [src/main/controlServer.ts:170-173](file://src/main/controlServer.ts#L170-L173)
...
- 关键设计点
  ...
  - **新增** 配置保存时调用 applyExcludeFilter()，确保排除状态实时同步到控制服务器

**更新** 增强了配置保存流程，实现了排除状态的实时同步：

**章节来源**
- [src/main/index.ts:1-120](file://src/main/index.ts#L1-L120)
- [src/main/index.ts:479-718](file://src/main/index.ts#L479-L718)
- [src/main/index.ts:213-326](file://src/main/index.ts#L213-326)
- [src/main/index.ts:55-148](file://src/main/index.ts#L55-L148)
- [src/main/index.ts:496-502](file://src/main/index.ts#L496-L502)
...
**更新** 新增 exclude filter 依赖关系，主进程现在需要调用控制服务器的 applyExcludeFilter() 方法。
...
- **新增** 排除过滤同步问题
  - 现象：桌面端排除分组后，手机端仍显示该分组
  - 原因：applyExcludeFilter() 未被正确调用或配置未同步
  - 解决：检查配置保存流程，确认 refreshConfig() 和 applyExcludeFilter() 都被调用
...
## 结论
移动端控制服务器以轻量 HTTP 服务形式嵌入主进程，通过回调机制与桌面端深度协作，实现了"手机即遥控器"的便捷体验。其设计兼顾了安全性（可选密码与限频）、一致性（共享扫描结果与互斥锁）、性能（缓存与 stream copy）与可维护性（模块化与测试覆盖）。

**更新** 新增的排除过滤系统进一步提升了用户体验，实现了桌面端与移动端界面的实时同步，用户无需手动刷新即可看到最新的分组状态。这一改进显著降低了操作门槛，提升了合并与投稿效率。
...
**更新** 分组 API 现已集成排除过滤功能，GET /api/groups 返回的分组列表会自动应用隐藏的文件夹排除。
...
- [src/main/controlServer.ts:593-673](file://src/main/controlServer.ts#L593-L673)
- [src/main/controlServer.ts:675-731](file://src/main/controlServer.ts#L675-L731)
- [src/main/controlServer.ts:338-343](file://src/main/controlServer.ts#L338-L343)